...
//    public PersonalChat toPersonalChat(ChatJpaEntity chatJpaEntity) {
//        List<UserId> members = getChatMembers(chatJpaEntity);
//
//        return new PersonalChat(
//                new ChatId(chatJpaEntity.getId()),
//                members,
//                chatJpaEntity.getLastMessageNumber()
//        );
//    }
//
    public GroupChat toGroupChat(ChatJpaEntity chatJpaEntity) {
        List<UserId> members = getChatMembers(chatJpaEntity);

        return new GroupChat(
                new ChatId(chatJpaEntity.getId()),
                chatJpaEntity.getName(),
                chatJpaEntity.getDescription(),
                new FileId(chatJpaEntity.getPhoto().getId()),
                members,
                chatJpaEntity.getLastMessageNumber()
        );
    }

    private List<UserId> getChatMembers(ChatJpaEntity chatJpaEntity) {
        List<UserId> members = new ArrayList<>();
        for (ChatMemberJpaEntity member : chatJpaEntity.getMembers()) {
            members.add(new UserId(member.getUser().getId()));
        }
        return members;
    }
//
...
//                members,
//                jpaEntity.getLastMessageNumber()
//        );
//    }
}
